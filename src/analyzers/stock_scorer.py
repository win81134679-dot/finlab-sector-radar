...
def score_stocks(
    ...
    #
    # ⚠️ 實測警示（2026-05-30 完整評分卡 point-in-time 回測，見 claude.md §9.4.1）：
    #   含輪動加分 vs 不含，持有1季贏 TAIEX 命中率 45.1% vs 45.2%（邊際 −0.1pp≈0）。
    #   即：① 完整評分卡命中率 ~45%，未過 50%、無顯著贏大盤 alpha；
    #       ② 輪動加分對命中率邊際 ≈ 0（改變選到哪些股，但勝率不變）。
    #   故輪動加分**刻意保留小權重(+1)**：它不傷害命中率、提供可解釋性（看盤情境），
    #   但**不是 alpha 來源**。真正選股決策以法人(燈2)+基本面(燈1/3)+品質過濾為主。
    #   **勿放大此 bonus 權重、勿單獨用輪動選股、勿宣稱它能擊敗大盤。**
    _rot_bonus = 0.0
    _rot_tags: List[str] = []
