...
兩者在 composite.py 各持 50% 權重，合計視語境中性或微正。

係數值域：-1.0（最強即時負衝擊）~ +1.0（最強即時正衝擊）
──────────────────────────────────────────────────────────────────────────
⚠️  本檔案修改後必須同步更新 frontend/lib/keywords.ts
    兩份為同步文件，不分主從
══════════════════════════════════════════════════════════════════════════
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# 多詞組（優先匹配，Python 3.7+ dict 保序）
# 貼文文字先掃 PHRASE_MATRIX，再掃 KEYWORD_MATRIX，允許重疊累加
# ══════════════════════════════════════════════════════════════════════════════
PHRASE_MATRIX: dict[str, dict[str, float]] = {
    # ── 貿易戰（最強訊號）
...
# ══════════════════════════════════════════════════════════════════════════════
# 單詞 / 短語關鍵詞（plain string, case-insensitive substring match）
# ══════════════════════════════════════════════════════════════════════════════
KEYWORD_MATRIX: dict[str, dict[str, float]] = {
    # ── 半導體 / 晶圓代工 ─────────────────────────────────────────────────────
    ...
    "huawei":            {"foundry": +0.5, "ic_design": -0.7},  # 對Huawei禁 → 台積電受益
    ...
    # ── 關稅（短期市場恐慌方向）────────────────────────────────────────────
    "tariff":            {"foundry": -0.7, "shipping": -0.5, "steel": -0.5,
                          "ic_design": -0.6, "display": -0.5, "ev_supply": -0.4},
    "tariffs":           {"foundry": -0.7, "shipping": -0.5, "steel": -0.5,
                          "ic_design": -0.6, "display": -0.5, "ev_supply": -0.4},
    "sanction":          {"ic_design": -0.8, "foundry": +0.4},
    "sanctions":         {"ic_design": -0.8, "foundry": +0.4},

    # ── 中國 ──────────────────────────────────────────────────────────────────
    "china":             {"ic_design": -0.5, "shipping": -0.4,
                          "display": -0.4, "ev_supply": -0.3},
    "chinese":           {"ic_design": -0.5, "shipping": -0.4,
                          "display": -0.4},
    ...
    "taiwan":            {"foundry": -0.3, "defense": +0.4},   # 地緣風險上升
    ...
    "apple":             {"pcb": +0.5, "foundry": +0.3},    # Apple供應鏈台灣受益
    ...
    "green":             {"solar": -0.5, "wind_energy": -0.5},  # 川普反對再生能源
    "solar":             {"solar": -0.6},
...
# ⚠️  本區修改後必須同步更新 frontend/lib/keywords.ts
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# 川普語境 VADER 自訂詞彙
# 覆蓋 vaderSentiment 預設詞彙，提升川普推文情緒辨識準確度
# 基於 Dhruvreddyp / SP500Prediction 的實驗結果調整
# ⚠️  本區修改後必須同步更新 frontend/lib/keywords.ts 的 TRUMP_CUSTOM_LEXICON
# ══════════════════════════════════════════════════════════════════════════════
TRUMP_VADER_LEXICON: dict[str, float] = {
    "tremendous":      +3.0,
    "disaster":        -3.0,
    "tariff":          -2.0,    # 市場對此字反應最敏感
    "tariffs":         -2.0,
    "beautiful":       +1.5,    # 川普說 beautiful 通常指 deal，正向
    "winning":         +2.0,
    "sanctions":       -2.5,
    "sanction":        -2.5,
    "deal":            +1.5,
