...
### 9.4.4 盤性 regime.ts 手訂權重逐項校準（2026-05-30，`scripts/backtest_regime_factors.py`）

把 regime.ts 各型態當選股因子回測（持有1季，baseline 39.6%）→ 校準手訂分數：

| 型態 | 原分數 | 實測 alpha | 樣本 | 判定 → 校準 |
|------|------|------|------|------|
| 連2漲停（大戶攻勢）| −2 | **+15.1%** | 53 | ❌ 方向相反（但 n=53 太小）→ **0**（中性，保留大戶標記）|
| 高位長上影線（派發）| −3 | +0.6% | 17,312 | ❌ ≈中性非派發 → **−1** |
| KDJ 低位金叉（啟動）| +2 | +1.7% | 23,518 | ⚠️ 方向對但弱 → **+1** |
| KDJ 高位死叉（轉弱）| −2 | +1.7% | 12,634 | ❌ 微正非轉弱 → **0** |

- **一致發現：regime.ts 系統性高估「派發/看空」型態**——量增不漲、長上影線、高位死叉**全部被判看空，但實測皆 ≈中性或微正**。已逐項校準 `regime.ts`（`analyzeKBar` + `calcKDJ`），UI 標籤加「回測」說明。
- **連2漲停 −2 方向錯**（回測偏多）但 n=53 樣本太小不能反向加分 → 中性化（移除無據重罰）。
- KDJ 數學本身正確，僅分數幅度校準。
- **結論**：regime.ts 經此校準後，看空型態不再過度懲罰；但此頁仍定位「看盤輔助」——多數型態 |alpha|<2pp（弱訊號），**真選股決策仍以投信籌碼為主**（§9.4.2/9.4.3）。媒體熱度/領頭股/499 因依賴當期排名/tick，未納入本次回測。
