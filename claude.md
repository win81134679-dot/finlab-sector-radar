...
**結論與系統定位**：
1. **輪動訊號的正確用法 = 選股「加分」（rotation bonus 疊在七燈+基本面+法人之上），不是單獨拿來月頻換板塊。** 見 `stock_scorer.py` 的輪動加分（上限僅 +1，刻意小）。
2. 注意：§9.4.1 的個股測試用**純 60 日動能代理**排名，非完整 `stock_scorer`（含基本面/法人/技術）。完整評分卡的命中率可能不同，但**在未經完整回測證實前，不得宣稱輪動能擊敗大盤**。
3. 沿用既有防禦：所有新訊號失敗**降級不中斷**；新閾值進 `config.py` 並標出處。
